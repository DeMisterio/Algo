...
agreement_words = [
    "Certainly", "Understood", "Absolutely", "Noted", "Of course", 
...
def replylogic():
    global findingtheplanning, pleasure, meating, Agrement
    findingtheplanning = re.search(patternofdate, corrected_text)
    findthegreeting = re.search(meeting_words_pattern, corrected_text)
    if findthegreeting:
        pleasure = True
        meating = listofgreetings[random.randint(0, len(listofgreetings) - 1)]
    else:
        pleasure = False
        meating = ""
    Agrement = agreement_words[random.randint(0, len(agreement_words) - 1 )]
...
def processplanning():
    ...
    tellhim(f"{meating + "!"} Algo is trying to understand your text: {corrected_text}")  # Выводим исправленный текст для диагностики
    minlenprompt = 5
    ...
    if "to" in corrected_text.split():
        ...
        if similarity > 60:
            ...
            tellhim(f"{Agrement}! I will remind you to {extracted_text} {findingtheplanning.group()}")
            thestart()
