...
def listen_vad_once():
    ...
    model = None
    try:
        # Load without compilation to bypass TF graph build issues during inference
        model = keras.models.load_model(MODEL_PATH, compile=False)
    except (OSError, IOError) as load_err:
        if Debugstat is True:
            print(f"Wake word model load failed: {load_err}. Attempting recovery...")
        ...
        try:
            model = keras.models.load_model(MODEL_PATH, compile=False)
        except (OSError, IOError) as second_err:
            if Debugstat is True:
                print(f"Repeated wake word model load failure: {second_err}")
            return "CRITICAL_ERROR"
    if model is None:
        if Debugstat is True:
            print("Wake word model unavailable; aborting listen loop.")
        return "CRITICAL_ERROR"
    vad = webrtcvad.Vad(2)
    p = pyaudio.PyAudio()
...
async def Voice_Initial_Check():
    if Umode == "Voice":
        ...
        if 120 < size_mb < 500:
            ...
            modelW = WhisperModel(str(model_folder), device="cpu", compute_type="float32")
...
def thestart():
    ...
    if message == "ERROR_NO_VOICE":
        ...
        try:
            ...
            loop.create_task(replylogic())
        except RuntimeError:
            # No running loop (e.g., called from plain sync context)
            asyncio.run(replylogic())
...
def cashe_list_creator(User_message):
    ...
    try:
        ...
        agent_creation_matches = [{"AgentCreation": match} for match in re.finditer(AlgoParser.agent_pattern, User_message,  re.IGNORECASE)]
        # searching_match = re.search(AlgoParser.search_pattern, User_message, re.IGNORECASE)
        ...
        # if searching_match:
        #     local_cache.append({"searching": searching_match})
        ...
        cashe_list = local_cache
        # searching = {"searching": searching_match}
...
async def replylogic():
    ...
    intent_box = []
    loop = asyncio.get_running_loop()
    intent_future = asyncio.create_task(Algo_Intelligence.task_certifier(User.message, agent))
    cache_future = asyncio.to_thread(cashe_list_creator, User.message)
    Prompt_analysis = await asyncio.gather(intent_future, cache_future, return_exceptions=True)

    result_AI_intent, result_cache_list = Prompt_analysis
    if isinstance(result_AI_intent, Exception):
        if Debugstat is True:
            print(f"Algo_Intelligence.task_certifier failed: {result_AI_intent}")
        result_AI_intent = {}
    if isinstance(result_cache_list, Exception):
        if Debugstat is True:
            print(f"cashe_list_creator failed: {result_cache_list}")
        cashe_list.clear()
        result_cache_list = []
    ...
    message_segments = []
    cache_count = len(cashe_list)
    if cache_count > 1:
        for cached_task in cashe_list:
            st_index, end_index = find_engine(User.message, cached_task)
            if st_index is None or end_index is None or end_index < st_index:
                if Debugstat is True:
                    print(f"Could not resolve segment bounds for task {cached_task}")
                continue
            segment = User.message[st_index:end_index + 1]
            if segment:
                message_segments.append(segment)
    elif cache_count == 1:
        key, value = next(iter(cashe_list[0].items()))
        if result_AI_intent:
            main_intent = result_AI_intent.get("Intent")
            str_index, end_index = find_engine(User.message, cashe_list[0])
            if str_index is None or end_index is None or end_index < str_index:
                segment = User.message
            else:
                segment = User.message[str_index:end_index + 1]
            if main_intent and main_intent != str(key):
                prefix = User.message[:str_index] if str_index is not None else ""
                suffix = User.message[end_index + 1:] if end_index is not None else ""
                task = (prefix + suffix).strip()
                if task:
                    message_segments.append(task)
                if segment:
                    message_segments.append(segment)
            else:
                if segment:
            ...
            message_segments.append(User.message)
    else:
        tot = True
        message_segments.append(User.message)
        Enclave_completing = False
    ...
    if len(message_segments) != 0:
        intent_box = []
        for task in message_segments:
            if tot is not True:
                try:
                    intent_box.append(await Algo_Intelligence.task_certifier(task, agent))
                except Exception as e:
                    if Debugstat is True:
                        print(f"Algo_Intelligence.task_certifier failed: {e}")
                    intent_box.append({})
            else:
    ...
    for i in intent_box:
        dict_intent = i
        if not isinstance(dict_intent, dict):
            if Debugstat is True:
                print(f"Unexpected intent format: {dict_intent}")
            continue
        main_intent = dict_intent.get("Intent")
        probability = dict_intent.get("Probability", 0.0) or 0.0
        text = dict_intent.get("Text", User.message)
        if Debugstat is True:
    ...
    if task_to_start == "AI_prompt":
        ...
        if CommonUtil.read_key_from_JSON("Umode") != 'Voice':
            ...
            if FileRenamingStatus == "Success":
                ...
                first_match = None
                if Nick_finding and "NameSet" in Nick_finding[0] and Nick_finding[0]["NameSet"]:
                    first_match = Nick_finding[0]["NameSet"][0]
                if first_match:
                    UMC = message[:first_match.start()] + UMC[end_index:]
            else:
                if Debugstat:
...
async def main_initializer():
    # 1) Ensure config is loaded first so globals like Uname/Ubotreference exist
    await config_initializer()
    
    # 2) Then load voice and Rasa in parallel (they may rely on config values)
    await asyncio.gather(Voice_Initial_Check(), rasa_loader())

    # 3) Now we are ready
    CommonUtil.play_sound("Ready_sound.mp3")

    # 4) Only after init is complete, start the main loop logic
    # thestart() is a synchronous function; call it directly
    thestart()
    # Keep the event loop alive so replylogic tasks created by thestart() can run
    try:
        while True:
